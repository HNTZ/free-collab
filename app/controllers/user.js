...
class UserManager {
          ...
          UserModel.updateOne(
            { username: user.username },
            {
              $set: {
                ...
                date_of_birth: new Date(User.date_of_birth),
                skills:
                  typeof User.skills === "object" ? User.skills : [User.skills]
              }
            }
          ).then(result => {
            if (!result.ok) {
              return reject({
                msg: "Une erreur a eu lieu au cours de la mise a jour."
              })
            } else {
  ...
  static async getUserName(_id) {
    return await UserModel.findOne({ _id }).then(
      user => `${user.username} (${user.firstname} ${user.lastname})`
    )
  }

  static async getUserSkills(_id) {
    return await UserModel.findOne({ _id }).then(user =>
      Promise.all(
        user.skills.map(async skill =>
          await SkillManager.getSkillById(skill).then(skill => skill)
        )
      )
    )
  }
